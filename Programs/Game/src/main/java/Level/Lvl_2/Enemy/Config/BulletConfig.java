...
public class BulletConfig {
    ...
    public static final double DEAD_DURATION = 2.5;

    public static Property getProperty(){
        Status status = new Status();
        Position position =  new Position(BulletConfig.INITIAL_X, BulletConfig.INITIAL_Y);
        HitBox hitBox = new HitBox(BulletConfig.WIDTH, BulletConfig.HEIGHT, position, BulletConfig.IS_BLOCKABLE, status);
        MovementState movementState = new MovementState(BulletConfig.SPEED, status);
        Health health = null;
